import os
import json

from django import template
from django.conf import settings
from django.utils.safestring import mark_safe

register = template.Library()

@register.simple_tag

def render_vite_bundle():
    
    """
    Template tag to render a vite bundle.
    Supposed to only be used in production.
    For development, see other files.
    """
    try:
        fd = open(f"{settings.VITE_APP_DIR}/dist/manifest.json", "r")
        manifest = json.load(fd)

    except:
        raise Exception(
            f"Vite manifest file not found or invalid. Maybe your {settings.VITE_APP_DIR}/dist/manifest.json file is empty?"
        )

    # images_tags = "".join(
    #         [
    #             f'<img src="/static/{manifest[file]["file"]}" alt="{file}" />'
    #             for file in manifest.get("assets/images/logos", {})  # Use .get() to handle missing keys
    #         ]
    #     )

    # imports_files = "".join(
    #     [
    #         f'<script type="module" src="/static/{manifest[file]["file"]}"></script>'
    #         for file in manifest["assets/images/logos/logoMdt.png"]
    #     ]
    # )

    # Genera etiquetas de script (si es necesario)
    #script_tag = f'<script type="module" src="/static/{manifest["index.html"]["file"]}"></script>'

    return mark_safe(
        f"""<script type="module" src="/static/{manifest['index.html']['file']}"></script>
        """
        #{imports_files}
    )